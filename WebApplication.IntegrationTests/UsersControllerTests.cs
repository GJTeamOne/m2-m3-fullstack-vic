...
namespace WebApplication.IntegrationTests
{
    public class UsersControllerTests : IClassFixture<WebApplicationFactory<Startup>>
    {
        ...
        [Theory]
        [AutoData]
        public async Task GivenInvalidUserId_WhenGettingUserById_ThenReturnBadRequest([Range(-99, -1)]int userId)
        {
            // Arrange
            HttpClient client = _factory.CreateClient();
        ...
        [Theory]
        [AutoData]
        public async Task GivenOutOfRangeUserId_WhenDeletingUser_ThenReturnNotFound(int userId)
        {
            // Arrange
            HttpClient client = _factory.CreateClient();
            var url = $"{client.BaseAddress}Users?id={userId}";
